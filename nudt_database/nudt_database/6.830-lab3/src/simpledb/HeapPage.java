...
public class HeapPage implements Page
{
	...
	{
		// some code goes here
		return Math.floorDiv((BufferPool.PAGE_SIZE * 8), (td.getSize() * 8 + 1));
	...
	{
		// some code goes here
		RecordId ri = t.getRecordId();
		if(ri.getPageId() != pid)
			throw new DbException("The Tuple isn't in this page");
		if(!getSlot(ri.tupleno()))
			throw new DbException("The Tuple is already empty");
		
		setSlot(ri.tupleno(), false);
	...
	public void addTuple(Tuple t) throws DbException
	{
		...
		if(td.equals(t.getTupleDesc()) == false)
			throw new DbException("TupleDesc is not match " + td + " but is " + t.getTupleDesc());
		
		int tupleno = NextEmptySlot();
		RecordId ri = new RecordId(pid, tupleno);
		tuples[tupleno] = t;
		t.setRecordId(ri);
	...
	public int NextEmptySlot() throws DbException
	{
		for(int i =0; i<numSlots;i++)
			if(((header[i/8] >> i%8) & 1) == 0)
				return i;
		throw new DbException("The process of Finding empty slot is fail");
	}
	/**
	 ...
	 */
	public boolean Judge_header(int posi)
	{
		byte tmp = (byte) ((header[posi / 8] >> (posi % 8)) & 1);
		return tmp == 1;
	}
	...
	{
		// some code goes here
		//System.out.println(" i is " + i + "value is " + value);
		header[i/8] = (byte) ((255-(1<<i%8)) & header[i/8]);
		if(value)
			header[i/8] += (1<< i%8);
		//if(Judge_header(i))
	...
	{
		// some code goes here
		ArrayList<Tuple> tpArrayList = new ArrayList<Tuple>();
		for (int i = 0; i < numSlots; i++)
			if (Judge_header(i))
				tpArrayList.add(tuples[i]);
		return tpArrayList.iterator();
	}
